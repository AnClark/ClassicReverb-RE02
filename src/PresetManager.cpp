...
{
    Preset p;
    p.roomSize = fUI->fParams[kRoomSize];
    p.damping  = fUI->fParams[kDamping];
    p.preDelay = fUI->fParams[kPreDelay];
    p.hiDamp   = fUI->fParams[kHiDamp];
    p.loCut    = fUI->fParams[kLoCut];
    p.earlyRef = fUI->fParams[kEarlyReflection];
    p.mix      = fUI->fParams[kMix];
    p.level    = fUI->fParams[kLevel];
    return p;
}
...
{
    DISTRHO_SAFE_ASSERT_RETURN(fUI != nullptr, )
    _triggerParamUpdate(kRoomSize,       preset.roomSize);
    _triggerParamUpdate(kDamping,        preset.damping);
    _triggerParamUpdate(kPreDelay,       preset.preDelay);
    _triggerParamUpdate(kHiDamp,         preset.hiDamp);
    _triggerParamUpdate(kLoCut,          preset.loCut);
    _triggerParamUpdate(kEarlyReflection, preset.earlyRef);
    _triggerParamUpdate(kMix,            preset.mix);
    _triggerParamUpdate(kLevel,          preset.level);
}
